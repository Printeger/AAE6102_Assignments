...
P = diag([1000,1000,1000,100,100,100,1000]);
...
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        navSolutions.satllitevelocity{currMeasNr}=satvelocity; %%add by YMD

        %% static SPP with Extended Kalman Filter --sbs
        if currMeasNr == 1
            X = xyzdt;
        else
            X(4)=0;  % (TODO:--sbs) fix the bug of rcv clk time update
        end
        ...
        % raw doppler velocity estimation by YMD
        est_rec_vel = rec_vel(navSolutions,trackResults,settings,currMeasNr);

        
        if currMeasNr == 1
            X = [xyzdt(1:3),0,0,0,xyzdt(4)]';
        else
            X(7)=0;  % rcv clk bias has been corrected after 1st epoch, so it should be 0 --sbs
        end

        % est_rec_vel=zeros(3,1);
        % est_rec_vel=zeros(3,1);
        [X_k, P_k]= my_EKF(X,P,est_rec_vel(1:3),clkCorrRawP,satPositions,settings);
        ...
        navSolutions.VX_kf(currMeasNr)  = X_k(4)+est_rec_vel(1);
        navSolutions.VY_kf(currMeasNr)  = X_k(5)+est_rec_vel(2);
        navSolutions.VZ_kf(currMeasNr)  = X_k(6)+est_rec_vel(3);         
